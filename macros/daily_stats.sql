{% macro daily_stats(source_name, table_name) %}
...
FROM
  ...
  joint_daily_stats_platform_info AS (
    SELECT daily_stats.* ,overall_info_joining.* EXCEPT(creative_id_1) FROM daily_stats LEFT JOIN overall_info_joining ON daily_stats.creative_id=overall_info_joining.creative_id_1
  ),
{% endmacro %}